...
void action_get(String request, String *body, byte *err) {     // Pour rÃ©cupÃ©rer des informations 
  ...
  if ((request.charAt(13) == 'd') || (request.charAt(13) == 'D')) {
    switch (pin) {                                            // Switch permettant de transformer prÃ©alablement les donnÃ©es avant l'envoi (Ã  Ã©viter, privilÃ©gier un traitement cÃ´tÃ© SARAH-serveur, pour Ã©viter la surcharge et ne pas dÃ©passer la limite de place de l'arduino)
      case 1:
      //*body += (digitalRead(pin)) + ln;
      *body += map((luminosite()), 0, 1023, 0, 100);
      break;
      case 2:
      //Serial.println("Cas GET D2");
      //*body += (digitalRead(pin)) + ln;
      if (enmarche)
      {
      if (!sens)
      ...
      else /**body += positionvolet;*/value=positionvolet;
      *body += value;
      Serial.print(F("Pos volet "));
      Serial.println(value);
      break;
